...
import fr.acinq.eclair.KotlinUtils.{OrderedSatoshi, javain2scala}
...
object TxPublisher {
  ...
  def adjustAnchorOutputChange(unsignedTx: ClaimLocalAnchorOutputTx, commitTx: Transaction, amountIn: Satoshi, currentFeerate: FeeratePerKw, targetFeerate: FeeratePerKw, dustLimit: Satoshi): ClaimLocalAnchorOutputTx = {
    ...
    val targetFee = weight2fee(targetFeerate, estimatedWeight) minus weight2fee(currentFeerate, commitTx.weight())
    val amountOut = dustLimit.max(amountIn minus targetFee)
    unsignedTx.copy(tx = unsignedTx.tx.updateOutputs(Lists.newArrayList(unsignedTx.tx.txOut.get(0).updateAmount(amountOut))))
  }
  ...
  def adjustHtlcTxChange(unsignedTx: HtlcTx, amountIn: Satoshi, targetFeerate: FeeratePerKw, commitments: Commitments): HtlcTx = {
    ...
    val changeAmount = amountIn minus dummySignedTx.tx.txOut.get(0).amount minus targetFee
    if (dummySignedTx.tx.txOut.size() == 2 && changeAmount >= commitments.localParams.dustLimit) {
      unsignedTx match {
        case htlcSuccess: HtlcSuccessTx => htlcSuccess.copy(tx = htlcSuccess.tx.updateOutputs(Lists.newArrayList(htlcSuccess.tx.txOut.get(0), htlcSuccess.tx.txOut.get(1).updateAmount(changeAmount))))
        case htlcTimeout: HtlcTimeoutTx => htlcTimeout.copy(tx = htlcTimeout.tx.updateOutputs(Lists.newArrayList(htlcTimeout.tx.txOut.get(0), htlcTimeout.tx.txOut.get(1).updateAmount(changeAmount))))
      }
    } else {
      unsignedTx match {
        case htlcSuccess: HtlcSuccessTx => htlcSuccess.copy(tx = htlcSuccess.tx.updateOutputs(Lists.newArrayList(htlcSuccess.tx.txOut.get(0))))
        case htlcTimeout: HtlcTimeoutTx => htlcTimeout.copy(tx = htlcTimeout.tx.updateOutputs(Lists.newArrayList(htlcTimeout.tx.txOut.get(0))))
      }
    }
  }
      ...
      txInfo match {
        case tx: HtlcSuccessTx =>
          commitments.localChanges.all.collectFirst {
            case u: UpdateFulfillHtlc if u.paymentPreimage.sha256() == tx.paymentHash => u.paymentPreimage
          }.flatMap(preimage => {
...
private class TxPublisher(nodeParams: NodeParams, watcher: akka.actor.ActorRef, client: ExtendedBitcoinClient, context: ActorContext[TxPublisher.Command])(implicit ec: ExecutionContext = ExecutionContext.Implicits.global) {
  ...
  private def publishLocalAnchorTx(txInfo: ClaimLocalAnchorOutputTx, commitments: Commitments): Future[ByteVector32] = {
    ...
    if (targetFeerate <= commitFeerate) {
      ...
      addInputs(txInfo, targetFeerate, commitments).flatMap(claimAnchorTx => {
        ...
        val commitInfo = ExtendedBitcoinClient.PreviousTx(signedClaimAnchorTx.input, signedClaimAnchorTx.tx.txIn.get(0).witness)
  ...
  private def addInputs(txInfo: ClaimLocalAnchorOutputTx, targetFeerate: FeeratePerKw, commitments: Commitments): Future[ClaimLocalAnchorOutputTx] = {
    ...
    val dummyChangeAmount = weight2fee(anchorFeerate, claimAnchorOutputMinWeight) plus dustLimit
    val txNotFunded = new Transaction(2, Nil, new TxOut(dummyChangeAmount, Script.pay2wpkh(PlaceHolderPubKey)) :: Nil, 0)
    client.fundTransaction(txNotFunded, FundTransactionOptions(anchorFeerate, lockUtxos = true)).flatMap(fundTxResponse => {
      ...
      val unsignedTx = txInfo.copy(tx = fundTxResponse.tx.updateInputs(txInfo.tx.txIn.get(0) +: fundTxResponse.tx.txIn))
      adjustAnchorOutputChange(unsignedTx, commitTx, fundTxResponse.amountIn plus AnchorOutputsCommitmentFormat.anchorAmount, commitFeerate, targetFeerate, dustLimit)
    })
  }
        ...
        if (targetFeerate <= currentFeerate) {
          ...
          addInputs(txInfo, targetFeerate, commitments).flatMap(unsignedTx => {
            ...
            client.signTransaction(signedHtlcTx.tx, Seq(inputInfo), allowIncomplete = true).flatMap(signTxResponse => {
              ...
              val completeTx = signedHtlcTx.tx.updateInputs(signedHtlcTx.tx.txIn.get(0) +: signTxResponse.tx.txIn.drop(1))
  ...
  private def addInputs(txInfo: HtlcTx, targetFeerate: FeeratePerKw, commitments: Commitments): Future[HtlcTx] = {
    ...
    val txNotFunded = txInfo.tx.updateInputs(Nil).updateOutputs(txInfo.tx.txOut.get(0).updateAmount(commitments.localParams.dustLimit) :: Nil)
    ...
    client.fundTransaction(txNotFunded, FundTransactionOptions(targetFeerate * weightRatio, lockUtxos = true, changePosition = Some(1))).map(fundTxResponse => {
      log.info(s"added ${fundTxResponse.tx.txIn.length} wallet input(s) and ${fundTxResponse.tx.txOut.size() - 1} wallet output(s) to ${txInfo.desc} spending commit input=${txInfo.input.outPoint.txid}:${txInfo.input.outPoint.index}")
      ...
      val txWithHtlcInput = fundTxResponse.tx.
        updateInputs(txInfo.tx.txIn ++ fundTxResponse.tx.txIn)
      .updateOutputs(txInfo.tx.txOut ++ fundTxResponse.tx.txOut.drop(1))
      ...
      adjustHtlcTxChange(unsignedTx, fundTxResponse.amountIn plus unsignedTx.input.txOut.amount, targetFeerate, commitments)
    })
  }
