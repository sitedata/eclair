...
import fr.acinq.eclair.blockchain.bitcoind.rpc.ExtendedBitcoinClient.{FundPsbtInput, FundPsbtOptions, FundPsbtResponse, FundTransactionOptions, ProcessPsbtResponse}
...
class PsbtSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  lazy val bitcoinClient = new ExtendedBitcoinClient(bitcoinrpcclient)
