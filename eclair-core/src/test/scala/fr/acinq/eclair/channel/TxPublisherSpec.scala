...
import fr.acinq.bitcoin.{BtcAmount, ByteVector32, MilliBtcDouble, OutPoint, PimpSatoshi, Satoshi, SatoshiLong, Script, ScriptFlags, ScriptWitness, SigVersion, Transaction, TxIn, TxOut}
...
class TxPublisherSpec extends TestKitBaseClass with AnyFunSuiteLike with BitcoindService with StateTestsHelperMethods with BeforeAndAfterAll with Logging {
  ...
  test("publish transactions with relative and absolute delays") {
    withFixture(Seq(500 millibtc), f => {
      ...
      val tx1 = {
        bitcoinWallet.fundTransaction(new Transaction(2, Nil, new TxOut(150000 sat, Script.pay2wpkh(priv.publicKey)) :: Nil, blockCount.get() + 5), lockUtxos = true, FeeratePerKw(250 sat)).pipeTo(probe.ref)
      ...
      val watchParentTx2 = alice2blockchain.expectMsgType[WatchParentTxConfirmed]
      assert(watchParentTx2.txId === tx1.txid)
      assert(watchParentTx2.minDepth === 2)
      createBlocks(2)
      txPublisher ! ParentTxConfirmed(watchParentTx2.childTx, tx1.txid)
      ...
      val watchParentTx3 = alice2blockchain.expectMsgType[WatchParentTxConfirmed]
      ...
      createBlocks(1)
      txPublisher ! ParentTxConfirmed(watchParentTx3.childTx, tx2.txid)
  ...
  test("publish transaction spending parent multiple times with different relative delays") {
    withFixture(Seq(500 millibtc, 500 millibtc), f => {
      ...
      val Seq(parentTx1, parentTx2) = (1 to 2).map(_ => {
        val outputs = Seq(new TxOut(outputAmount, Script.pay2wpkh(priv.publicKey)), new TxOut(outputAmount, Script.pay2wpkh(priv.publicKey)))
        bitcoinWallet.fundTransaction(new Transaction(2, Nil, outputs, 0), lockUtxos = true, FeeratePerKw(250 sat)).pipeTo(probe.ref)
        val funded = probe.expectMsgType[FundTransactionResponse].tx
        bitcoinWallet.signTransaction(funded).pipeTo(probe.ref)
        probe.expectMsgType[SignTransactionResponse].tx
      })
      ...
      val tx = {
        ...
        val inputs = Seq(
          new TxIn(new OutPoint(parentTx1, outputIndexes1.head), Nil, 1),
          new TxIn(new OutPoint(parentTx1, outputIndexes1.last), Nil, 2),
          new TxIn(new OutPoint(parentTx2, outputIndexes2.head), Nil, 3),
          new TxIn(new OutPoint(parentTx2, outputIndexes2.last), Nil, 4),
        )
        val unsigned = new Transaction(2, inputs, new TxOut(450000 sat, Script.pay2wpkh(priv.publicKey)) :: Nil, 0)
        (0 to 3).foldLeft(unsigned) {
          case (current, i) =>
            ...
            current.updateWitness(i, new ScriptWitness().push(sig).push(priv.publicKey.value))
        }
      }
      ...
      val watches = Seq(
        alice2blockchain.expectMsgType[WatchParentTxConfirmed],
        alice2blockchain.expectMsgType[WatchParentTxConfirmed],
      )
      ...
      createBlocks(2)
      txPublisher ! ParentTxConfirmed(w2.childTx, w2.txId)
  ...
  test("commit tx feerate too low, not enough wallet inputs to increase feerate") {
    withFixture(Seq(10.1 millibtc), f => {
      ...
      val targetFee: PimpSatoshi = Transactions.weight2fee(TestConstants.feeratePerKw, (mempoolTx1.weight + mempoolTx2.weight).toInt)
      val actualFee: PimpSatoshi = mempoolTx1.fees plus mempoolTx2.fees
  ...
  test("commit tx feerate too low, spending anchor output") {
    withFixture(Seq(500 millibtc), f => {
      ...
      val targetFee: PimpSatoshi = Transactions.weight2fee(TestConstants.feeratePerKw, mempoolTxs.map(_.weight).sum.toInt)
      ...
      for (_ <- 1 to 100) {
        ...
        if (adjustedTx.tx.txOut.head.amount > dustLimit) {
          ...
          val signedTx = {
            ...
            val signedWalletInputs = anchorSigned.tx.txIn.tail.map(txIn => txIn.updateWitness(
              new ScriptWitness().push(Scripts.der(Transactions.PlaceHolderSig)).push(Transactions.PlaceHolderPubKey.value))
            )
          ...
          val targetFee: PimpSatoshi = Transactions.weight2fee(TestConstants.feeratePerKw, signedTx.weight() + commitTx.weight()) minus Transactions.weight2fee(commitFeerate, commitTx.weight())
  ...
  test("htlc tx feerate high enough, not adding wallet inputs") {
    withFixture(Seq(500 millibtc), f => {
      ...
      val htlcSuccessTargetFee: PimpSatoshi = Transactions.weight2fee(currentFeerate, htlcSuccessTx.weight.toInt)
      ...
      val htlcTimeoutTargetFee: PimpSatoshi = Transactions.weight2fee(currentFeerate, htlcTimeoutTx.weight.toInt)
  ...
  test("htlc tx feerate too low, not enough wallet inputs to increase feerate") {
    withFixture(Seq(10.1 millibtc), f => {
      ...
      // The HTLC-success tx will be immediately published.
      val htlcSuccessTx = getMempoolTxs(1).head
      val htlcSuccessTargetFee: PimpSatoshi = Transactions.weight2fee(TestConstants.feeratePerKw, htlcSuccessTx.weight.toInt)
      assert(htlcSuccessTargetFee * 0.9 <= htlcSuccessTx.fees && htlcSuccessTx.fees <= htlcSuccessTargetFee * 1.1, s"actualFee=${htlcSuccessTx.fees} targetFee=$htlcSuccessTargetFee")
    })
  }
  ...
  test("htlc tx feerate too low, adding wallet inputs") {
    withFixture(Seq(500 millibtc), f => {
      ...
      createBlocks(144)
      val htlcTimeoutTx = getMempoolTxs(1).head
      val htlcTimeoutTargetFee: PimpSatoshi = Transactions.weight2fee(TestConstants.feeratePerKw, htlcTimeoutTx.weight.toInt)
      assert(htlcTimeoutTargetFee * 0.9 <= htlcTimeoutTx.fees && htlcTimeoutTx.fees <= htlcTimeoutTargetFee * 1.1, s"actualFee=${htlcTimeoutTx.fees} targetFee=$htlcTimeoutTargetFee")
    })
  }
  ...
  test("htlc tx sent after commit tx confirmed") {
    withFixture(Seq(500 millibtc), f => {
      ...
      alice2blockchain.expectMsgType[WatchTxConfirmed] // commit tx
      alice2blockchain.expectMsgType[WatchTxConfirmed] // claim main output
      alice2blockchain.expectMsgType[WatchOutputSpent] // alice doesn't have the preimage yet to redeem the htlc but she watches the output
      alice2blockchain.expectNoMessage(100 millis)
  ...
  test("adjust htlc tx change amount", Tag("fuzzy")) {
    withFixture(Seq(500 millibtc), f => {
      ...
      for (_ <- 1 to 100) {
        ...
        val unsignedHtlcTimeoutTx = htlcTimeout.txInfo.asInstanceOf[HtlcTimeoutTx].copy(tx = htlcTimeout.tx
          .updateInputs(htlcTimeout.tx.txIn ++ walletInputs)
          .updateOutputs(htlcTimeout.tx.txOut ++ Seq(changeOutput))
        )
        for (unsignedTx <- Seq(unsignedHtlcSuccessTx, unsignedHtlcTimeoutTx)) {
          ...
          if (adjustedTx.tx.txOut.size == 2) {
            ...
            val targetFee: PimpSatoshi = Transactions.weight2fee(targetFeerate, signedTx.weight())
            val actualFee: PimpSatoshi = totalAmountIn minus signedTx.txOut.map(_.amount).sum
