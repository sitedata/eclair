...
class NegotiatingStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  def testClosingSigned(f: FixtureParam): Unit = {
    ...
    if (alice.stateData.asInstanceOf[DATA_NEGOTIATING].commitments.channelFeatures.hasFeature(Features.OptionUpfrontShutdownScript)) {
      ...
      val expectedLocalScript = new fr.acinq.bitcoin.ByteVector(alice.stateData.asInstanceOf[DATA_NEGOTIATING].commitments.localParams.defaultFinalScriptPubKey.toArray)
      val expectedRemoteScript = new fr.acinq.bitcoin.ByteVector(bob.stateData.asInstanceOf[DATA_NEGOTIATING].commitments.localParams.defaultFinalScriptPubKey.toArray)
