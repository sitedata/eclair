...
class RouteCalculationSpec extends AnyFunSuite with ParallelTestExecution {
  ...
  test("find the k-shortest paths in a graph, k=4") {
    ...
    val fourShortestPaths = Graph.yenKshortestPaths(g1, d, f, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 4, NO_WEIGHT_RATIOS, 0, noopBoundaries, false)
    ...
    val threeShortestPaths = Graph.yenKshortestPaths(g2, d, f, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 4, NO_WEIGHT_RATIOS, 0, noopBoundaries, false)
  ...
  test("find the k shortest path (wikipedia example)") {
    ...
    val twoShortestPaths = Graph.yenKshortestPaths(graph, c, h, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 2, NO_WEIGHT_RATIOS, 0, noopBoundaries, false)
  ...
  test("terminate looking for k-shortest path if there are no more alternative paths than k, must not consider routes going back on their steps") {
    ...
    val foundPaths = Graph.yenKshortestPaths(graph, a, f, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 3, NO_WEIGHT_RATIOS, 0, noopBoundaries, false)
  ...
  test("select a random route below the requested fee") {
    ...
    for (_ <- 0 to 10) {
      ...
      val weightedPath = Graph.pathWeight(a, route2Edges(routes.head), DEFAULT_AMOUNT_MSAT, 400000, NO_WEIGHT_RATIOS, false)
