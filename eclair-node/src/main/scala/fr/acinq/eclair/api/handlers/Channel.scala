    ...
    formFields(nodeIdFormParam, "fundingSatoshis".as[Satoshi], "pushMsat".as[MilliSatoshi].?, "channelType".?, "fundingFeerateSatByte".as[FeeratePerByte].?, "channelFlags".as[Int].?, "openTimeoutSeconds".as[Timeout].?) {
      (nodeId, fundingSatoshis, pushMsat, channelType, fundingFeerateSatByte, channelFlags, openTimeout_opt) =>
        val (channelTypeOk, channelType_opt) = channelType match {
          ...
          case Some(str) if str == ChannelTypes.AnchorOutputsZeroFeeHtlcTxs.toString => (true, Some(ChannelTypes.AnchorOutputsZeroFeeHtlcTxs))
          case Some(_) => (false, None)
          case None => (true, None)
        }
        if (!channelTypeOk) {
          reject(MalformedFormFieldRejection("channelType", s"Channel type not supported: must be ${ChannelTypes.Standard.toString}, ${ChannelTypes.StaticRemoteKey.toString}, ${ChannelTypes.AnchorOutputs.toString} or ${ChannelTypes.AnchorOutputsZeroFeeHtlcTxs.toString}"))
        } else {
          complete {
