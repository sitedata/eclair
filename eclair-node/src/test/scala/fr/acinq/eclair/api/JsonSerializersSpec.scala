...
class JsonSerializersSpec extends AnyFunSuite with Matchers {
  ...
  test("TransactionWithInputInfo serializer") {
    ...
    val dummyInputInfo = InputInfo(new OutPoint(ByteVector32.Zeroes, 0), new TxOut(new Satoshi(0), Array.emptyByteArray), Nil)
    ...
    val closingTxWithLocalOutput = ClosingTx(dummyInputInfo, closingTx, Some(OutputInfo(1, new Satoshi(15000), hex"deadbeef")))
